...
def load_video_frames(
    ...
    if images is not None:
        # images is np.ndarray with shape (F, H, W, C) in uint8
        images = torch.from_numpy(images).float()          # (F, H, W, C) in [0..255]
        orig_h, orig_w = int(images.shape[1]), int(images.shape[2])

        images = images / 255.0                            # match JPEG path scaling
        images = rearrange(images, "f h w c -> f c h w")   # (F, C, H, W)
        images = F.interpolate(images, (image_size, image_size), mode="bilinear")

        # IMPORTANT: keep true video size for later upsampling
        video_height, video_width = orig_h, orig_w
